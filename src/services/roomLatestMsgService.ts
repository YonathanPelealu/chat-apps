...
const createLatestMsgInRoom = async (
	room_id: string,
	user_id: string,
	message_id: string
): Promise<any> => {
...
const updateLatestMsgInRoom = async (
	room_id: string,
	user_id: string,
	message_id: string
): Promise<any> => {
	try {
		let response;
		const query = `
            UPDATE room_latest_msg 
            SET message_id = $1, user_id = $2, room_id = $3 
            WHERE room_latest_msg.room_id = $3`;
...
const getLatestMessageForRoom = async (
	room_id: string
